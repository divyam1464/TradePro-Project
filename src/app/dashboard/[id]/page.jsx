...
const StockChart = ({ stock }) => {
  ...
  return (
    <motion.div {...fadeInUp} className="bg-gray-800 p-6 rounded-lg shadow-lg my-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4">
        <div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <motion.span
              className={`text-sm flex items-center ${change.value >= 0 ? "text-green-500" : "text-red-500"}`}
        ...
        <div className="flex space-x-2 mt-4 md:mt-0">
          <motion.button className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition-colors flex items-center cursor-pointer" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
            <PlusCircle className="inline-block mr-2" size={16} />Create Alert
          </motion.button>
          <motion.button className="bg-gray-700 text-white px-4 py-2 rounded hover:bg-gray-600 transition-colors flex items-center cursor-pointer" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
      ...
      <div className="flex justify-between mt-4 overflow-x-auto overflow-hidden">
        {["5M", "10M", "15M", "30M", "1H"].map((range) => (
          <motion.button
            key={range}
            className={`text-sm ${timeRange === range ? "text-blue-500" : "text-gray-300"} hover:text-blue-500 transition-colors flex items-center cursor-pointer`}
            onClick={() => setTimeRange(range)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.9 }}
          >
      ...
      <table className="w-full text-left">
        ...
        <tbody>
          {options.map((option, index) => (
            <motion.tr
              key={index}
              className="border-b border-gray-700" {...fadeInUp}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
